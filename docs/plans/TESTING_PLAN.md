...
- **Static checks** (lint/format): always
...
- **Screenshot tests**: targeted “goldens” (not everything)
...
- Inject `NowProvider` so time-sensitive features are deterministic.
- Use `kotlinx-coroutines-test` with a test dispatcher.
- If you randomize suggestions, inject a seeded RNG.

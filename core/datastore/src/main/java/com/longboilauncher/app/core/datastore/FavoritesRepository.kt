...
class FavoritesRepository @Inject constructor(
    ...
    val favorites: Flow<List<FavoriteEntry>> = dataStore.data
        .catch { exception ->
    ...
    suspend fun addFavorite(appEntry: AppEntry) {
        dataStore.updateData { currentSettings ->
            val alreadyExists = currentSettings.favoritesList.any {
            ...
            val newFavorite = FavoriteApp.newBuilder()
    ...
    suspend fun removeFavorite(favoriteId: String) {
        dataStore.updateData { currentSettings ->
            val updatedFavorites = currentSettings.favoritesList.filter {
    ...
    suspend fun reorderFavorites(favoriteIds: List<String>) {
        dataStore.updateData { currentSettings ->
            val reordered = favoriteIds.mapNotNull { id ->
